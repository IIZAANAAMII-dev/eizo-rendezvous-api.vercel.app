...
export default function WidgetPage() {
  ...
  const fetchAvailableSlots = async (date: Date) => {
    ...
    try {
      const response = await fetch(`/api/public/availability/${selectedOrganizer?.id}?date=${dateStr}`);
      const data = await response.json();
      setAvailableDays([{
        date: dateStr,
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white">
      <div className="max-w-6xl mx-auto px-6 py-12">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">EIZO Booking Widget</h1>
          <p className="text-gray-600">Choisissez un expert pour réserver un rendez-vous</p>
        </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4 mb-6">
                      <Avatar
                        ...
                        initials={getInitials(selectedOrganizer.name)}
                        size="xl"
                        status="online"
                      />
                      <div>
                        <h2 className="text-xl font-semibold text-gray-900">{selectedOrganizer.name}</h2>
                        <p className="text-sm text-gray-600">{selectedOrganizer.specialty || 'Expert EIZO'}</p>
                      </div>
                    </div>
